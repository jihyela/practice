...
public class PagingBean {
	 ...
	 * 페이지 당 보여줄 게시물 수 
	 */
	private int contentNumberPerPage=CommonConstants.CONTENT_NUMBER_PER_PAGE;//5
	/**
	 * 페이지 그룹 당 페이지 수 
	 */
	private int pageNumberPerPageGroup=CommonConstants.PAGE_NUMBER_PER_PAGEGROUP;//4
	/**
	 * 현재 db에 저장된 총 게시물 수 
	 */
	...
	public int getTotalPage(){		
		int totalPage=0;
		if(this.totalContents % this.contentNumberPerPage == 0) {
			totalPage = this.totalContents / this.contentNumberPerPage;
		} else if(this.totalContents % this.contentNumberPerPage > 0) {
	...
	public int getTotalPageGroup(){		
		...
		if(this.getTotalPage() % this.pageNumberPerPageGroup == 0) {
			totalPageGroup = this.getTotalPage()/this.pageNumberPerPageGroup;
		} else if(this.getTotalPage() % this.pageNumberPerPageGroup > 0) {
	...
	public int getNowPageGroup(){		
		int nowPageGroup=0;
		if(this.nowPage % this.pageNumberPerPageGroup == 0) {
			nowPageGroup = this.nowPage/this.pageNumberPerPageGroup;
		} else if(this.nowPage % this.pageNumberPerPageGroup > 0) {
